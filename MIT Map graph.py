...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        output = str(self.src) + '->' + str(self.dest) + ' (' + str(self.total_distance) +', ' + str(self.outdoor_distance) +')'
        return output
...
class Digraph(object):
    ...
    def __init__(self):
        self.nodes = set([]) #set() is an unordered collection with no duplicate elements
    ...
    def add_edge(self, edge):
        ...
        src = edge.src
        dest = edge.dest
        if Digraph.has_node(self, src) and Digraph.has_node(self, dest):
            self.edges[src].append(edge)
        else:
            raise ValueError('either one of the nodes associated with the edge is not in the  graph.')
